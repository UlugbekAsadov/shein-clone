...
export function useSimilarProducts(id: number | undefined) {
  ...
  return useQuery<{
    ...
    queryKey: ["similar-products", id, lang, currency],
    enabled: id != null,
    queryFn: async () => {
      try {
        const result = await productDetailApi.getSimilarProducts(id as number);
        return {
